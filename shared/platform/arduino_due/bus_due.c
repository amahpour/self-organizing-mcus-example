/**
 * @file bus_due.c
 * @brief Arduino Due hardware serial bus implementation
 *
 * Uses hardware Serial1 instead of SoftwareSerial for reliable communication.
 * This implementation works with Arduino Due's multiple hardware UART ports.
 */
...
int bus_create(Bus** bus, uint8_t node_index, uint8_t rx_pin, uint8_t tx_pin) {
    ...
    (void) rx_pin;      // Hardware serial pins are fixed
    (void) tx_pin;      // Hardware serial pins are fixed

    Bus* b = (Bus*) malloc(sizeof(Bus));
    if (!b)
        return -1;

    // Use Serial1 (pins 19 RX, 18 TX) on Arduino Due
    b->serial = &Serial1;
