...
int bus_send(Bus* bus, const Frame* frame) {
    if (!bus || !bus->serial || !frame) return -1;
    ...
    while ((hal_millis() - start) < timeout_ms) {
        if (bus->serial->available()) {
            ...
            if (b == SOF) {
                ...
                // Read fixed header
                if (!read_byte(bus->serial, &frame->type, timeout_ms)) return 0;
                if (!read_byte(bus->serial, &frame->source, timeout_ms)) return 0;
                if (!read_byte(bus->serial, &frame->payload_len, timeout_ms)) return 0;
                
                if (frame->payload_len > MAX_PAYLOAD_SIZE) return 0;
                
                // Read payload
                for (uint8_t i = 0; i < frame->payload_len; ++i) {
                    if (!read_byte(bus->serial, &frame->payload[i], timeout_ms)) return 0;
                }
                
                // Read checksum
                if (!read_byte(bus->serial, &frame->checksum, timeout_ms)) return 0;
